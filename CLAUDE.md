...
**Agent harness alt path.** When `AGENT_HARNESS_PROVIDER` is set to `groq`, `lm-studio`, or `claude-cli`, `/api/process-workout` swaps the single-shot vision call for a tool-use loop (model iterates `searchCatalog` / `getExerciseDetails` / `expandAbbreviations`, commits via terminal `proposeWorkout`). Default `off` — single-shot path unchanged. Response shape stays byte-compatible with the existing review/sync flow. Full details: `docs/ARCHITECTURE.md` § Agent harness.
...
- `lib/agents/` — tool-use harness (feature-flagged via `AGENT_HARNESS_PROVIDER`). `tools.ts` is the single source of truth for tool defs (handlers + JSON Schema); `match-loop.ts` drives the iterative path; `providers/{claude-cli,groq,lm-studio}.ts` are adapters. Tests in `lib/agents/__tests__/`.
- `lib/workout-set-builder.ts` — shared switch over `Exercise.type` → `WorkoutSet`. Used by both `lib/groq/helpers.ts` (single-shot path) and `lib/agents/tools.ts` (agent path). Don't reimplement set-shape conversion.
...
- New lib module: pick a domain (`hevy/`, `groq/`, `embeddings/`, `agents/`) or place at `lib/` root if cross-cutting. Don't recreate sibling utils that already exist in `lib/utils.ts` or `lib/agents/env.ts` (envInt/envFloat).
