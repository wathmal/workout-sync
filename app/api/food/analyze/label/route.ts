...
import { fmaAnalyzeLabel } from "@/lib/food/fma";
import { preparePhoto } from "@/lib/food/photo-prep";

/**
 * Body: { imageBase64, filename?, mimeType?, locale?, context? }
 * Returns: FMA analyze/label response (per_serving items) + EXIF date (if any).
 * Mirrors analyze/photo — same HEIC/EXIF/resize prep, distinct FMA endpoint.
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const analyze = await fmaAnalyzeLabel(prepared.base64, { locale, context });

    return NextResponse.json({
      ...analyze,
      exifDate: prepared.exifDateIso,
      // HEIC is unrenderable in <img>; hand back the transcoded JPEG so the
      // client can swap it into the preview (matches the photo flow).
